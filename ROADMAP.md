...
|---|---|---|---|
| No tests, no CI gate | High | Med | `pytest`/`pytest-asyncio`/`httpx` are in `backend/requirements.txt` but zero test files exist anywhere in the repo. Both `.github/workflows/*-AutoDeployTrigger-*.yml` files are Azure-generated deploy-only triggers (checkout → Azure login → build & push) — no lint or test step. Anything pushed to `main` ships straight to prod unchecked. **Fix direction**: add a basic pytest suite (health endpoint, session-id validation, chunking logic) and a CI job that runs it as a gate before the existing deploy trigger. |
...
- **Admin/analytics dashboard** surfacing the RAG logs already being collected in Azure Table Storage (`rag_logger.py`) — currently write-only; nothing reads this data back today.
